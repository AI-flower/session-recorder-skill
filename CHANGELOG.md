...
### Fixed
- **社区搜索 query 使用 AI 翻译后的 goal 导致匹配失败**：`<goal>` 占位符定义不明确，AI 将用户中文输入翻译为英文后发送，语义搜索匹配度大幅下降（实测：原文 0.72 vs 翻译后可能 < 0.3）。改为明确要求使用用户原始输入 `<user_raw_input>` 作为 query，禁止翻译/改写
- **社区搜索请求和响应无法审计**：只记录了 decision 结论，未记录实际发送的 query 和 API 返回内容。新增强制要求：decision 条目的 reason 字段必须包含 `query:` 和 `response:` 信息
